...
export const stockService = {
  stockIn: async (dto: StockInCreateDto): Promise<void> => {
    await apiClient.post('/stock/in', dto);
  },
  stockOut: async (dto: StockOutCreateDto): Promise<void> => {
    await apiClient.post('/stock/out', dto);
  },
  getBalance: async (): Promise<StockBalanceDto[]> => {
    const res = await apiClient.get('/stock/balance');
  ...
  getStockInHistory: async (): Promise<StockInDto[]> => {
    const res = await apiClient.get('/stock/in/history');
    return res.data?.data ?? res.data ?? [];
  },
  getStockOutHistory: async (): Promise<StockOutDto[]> => {
    const res = await apiClient.get('/stock/out/history');
    return res.data?.data ?? res.data ?? [];
  },
};